...
#+begin_src yaml
  proxy:
    ...
    ports:
      ...
      - 443:443
    volumes:
      - /exosx20/proxy/certs:/etc/nginx/certs:ro
      - /exosx20/proxy/vhost.d:/etc/nginx/vhost.d
      - /exosx20/proxy/html:/usr/share/nginx/html
...
#+begin_src yaml
  acme:
    ...
    depends_on:
      - proxy
    volumes:
      - /exosx20/proxy/certs:/etc/nginx/certs
      - /exosx20/proxy/acme:/etc/acme.sh
      - /exosx20/proxy/vhost.d:/etc/nginx/vhost.d
      - /exosx20/proxy/html:/usr/share/nginx/html
...
#+begin_src yaml :tangle no
  wireguard-ui:
    ...
    cap_add:
      - NET_ADMIN
    volumes:
      - /exosx20/wireguard-ui:/app/db
      - /exosx20/wireguard:/etc/wireguard
    environment:
      - SENDGRID_API_KEY
      - EMAIL_FROM_ADDRESS
      - EMAIL_FROM_NAME
...
#+begin_src yaml :tangle no
  transmission:
    ...
    ports:
      ...
      - ${TRANSMISSION_PEER_PORT}:${TRANSMISSION_PEER_PORT}/udp
    volumes:
      - /exosx20/transmission/config:/config
      - /exosx20/transmission/watch:/watch
      - /exosx20/transmission/downloads:/downloads
      - /exosx20/media:/downloads/media
    environment:
      - VIRTUAL_HOST=${TRANSMISSION_HOST}
      - VIRTUAL_PORT=${TRANSMISSION_WEBUI_PORT}
      - LETSENCRYPT_HOST=${TRANSMISSION_HOST}
...
#+begin_src yaml
  transmission:
    ...
    volumes:
      - /exosx20/transmission/config:/config/transmission-home
      - /exosx20/transmission/watch:/watch
      - /exosx20/transmission/downloads:/downloads
      - /exosx20/media:/downloads/media
    environment:
...
#+begin_src yaml
  nextcloud-app:
    ...
    depends_on:
      ...
      - nextcloud-redis
    volumes:
      - /exosx20/nextcloud/app:/var/www/html
...
#+begin_src yaml
  nextcloud-server:
    ...
    volumes:
      - /exosx20/nextcloud/app:/var/www/html:ro
    environment:
...
#+begin_src yaml
  nextcloud-cron:
    ...
    volumes:
      - /exosx20/nextcloud/app:/var/www/html
      - /exosx20/nextcloud/cron:/var/spool/cron/crontabs
    networks:
      - nextcloud
#+end_src
...
#+begin_src yaml
  nextcloud-db:
    ...
    volumes:
      - /exosx20/nextcloud/db:/var/lib/mysql
    environment:
...
#+begin_src yaml
  hass:
    ...
    volumes:
      - /exosx20/home-assistant/config:/config
...
#+begin_src yaml
  zwave-js-ui:
    ...
    volumes:
      - /exosx20/home-assistant/zwave:/usr/src/app/store
    devices:
...
#+begin_src yaml
  piper:
    ...
    ports:
      - 10200:10200
    volumes:
      - /exosx20/home-assistant/wyoming-piper:/data
...
#+begin_src yaml
  whisper:
    ...
    ports:
      - 10300:10300
    volumes:
      - /exosx20/home-assistant/wyoming-whisper:/data
...
#+begin_src yaml
  grafana:
    ...
    volumes:
      - /exosx20/grafana/app:/var/lib/grafana
    networks:
      - grafana
      - hass
#+end_src
...
#+begin_src yaml
  grafana-influxdb:
    ...
    volumes:
      - /exosx20/grafana/influxdb:/var/lib/influxdb2
    networks:
      - grafana
      - hass
#+end_src
...
#+begin_src yaml
  plex:
    ...
    ports:
      ...
      - 32414:32414/udp
    volumes:
      - /exosx20/plex/config:/config
      - /exosx20/plex/transcode:/transcode
      - /exosx20/media:/data
    environment:
...
#+begin_src yaml
  jellyfin:
    ...
    ports:
      - 8096:8096/tcp
    volumes:
      - /exosx20/jellyfin/config:/config
      - /exosx20/jellyfin/cache:/cache
      - /exosx20/media:/media:ro
    networks:
      - proxy-tier
      - hass
#+end_src
...
#+begin_src yaml
  asf:
    ...
    restart: unless-stopped
    volumes:
      - /exosx20/archi-steam-farm:/app/config
    environment:
...
#+begin_src yaml
  webtrees:
    ...
    depends_on:
      - webtrees-db
    volumes:
      - /exosx20/webtrees/data:/var/www/webtrees/data
      - /exosx20/webtrees/themes:/var/www/webtrees/modules_v4
    environment:
      # proxy
...
#+begin_src yaml
  webtrees-db:
    ...
    command: --transaction-isolation=READ-COMMITTED --log-bin=binlog --binlog-format=ROW
    volumes:
      - /exosx20/webtrees/db:/var/lib/mysql
    environment:
      - MARIADB_AUTO_UPGRADE=1
      - MARIADB_DISABLE_UPGRADE_BACKUP=1
...
#+begin_src yaml
  digikam-db:
    ...
    volumes:
      - /exosx20/digikam/db:/var/lib/mysql
    environment:
